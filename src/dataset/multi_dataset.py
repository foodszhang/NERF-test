    ...
    def __init__(self, path, n_rays=1024, type="train", device="cuda"):    
        ...
        if type == "train":
            ...
            for i, data in enumerate(all_data):
                ...
                self.data[i]['n_samples'] = data["numTrain"]
            # (256, 256, 2)
                # (256, 256, 2) -> (256 x 256, 2)
                self.data[i]['coords'] = torch.reshape(coords, [-1, 2])
                # 也有 raw，但是和 rays 并不对应
                self.data[i]['image'] = torch.tensor(data["image"], dtype=torch.float32, device=device)                 # [128, 128, 128]
                self.data[i]['voxels'] = torch.tensor(self.get_voxels(self.geo), dtype=torch.float32, device=device)    # [128, 128, 128, 3]]
        elif type == "val":
            for i, data in enumerate(all_data):
                self.data.append({})
                self.data[i]['projs']= torch.tensor(data["train"]["projections"], dtype=torch.float32, device=device) # [50, 256, 256]
                ...
                self.data[i]['voxels'] = torch.tensor(self.get_voxels(self.geo), dtype=torch.float32, device=device)
            # stx()
