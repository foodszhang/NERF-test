...
class TIGREDataset_MLG(Dataset):
    ...
    def __init__(
        ...
        if type == "train":
            ...
            rays = self.get_rays(
                angles, self.geo, device
            )  # [50, 256, 256, 6] 在每一个角度下获取射线的原点和方向 每一个像素点对应的ray
            # stx()
            """
            ...
            for i in range(len(nei_angles)):
                nei_rays = self.get_rays(
                    nei_angles[i], self.geo, device
                )  # [100, 256, 256, 6]
                self.nei_rays[i] = torch.cat(
                    [
                        nei_rays,
                        torch.ones_like(nei_rays[..., :1]) * self.near,
                        torch.ones_like(nei_rays[..., :1]) * self.far,
                    ],
                    dim=-1,
                )
            ...
            self.image = torch.tensor(
                data["image"], dtype=torch.float32, device=device
            )  # [128, 128, 128]
    ...
    def __getitem__(self, index):
        if self.type == "train":
            ...
            neighbors_rays = (
                self.nei_rays[i][index] for i in range(len(self.nei_rays))
            )  # [256, 256, 8]
            ...
            neighbors_rays = (
                ray_window_partition(rays, self.window_size) for rays in neighbors_rays
            )  # [256, 256, 8] -> [64, 32, 32, 8]
            ...
            # stx()
            selected_rays_window_valid = torch.concat(
                [selected_rays_window, selected_rays_else], dim=0
            )  # [num, 8]
            selected_projs_window_valid = torch.concat(
                [selected_projs_window, selected_projs_else], dim=0
            )  # [num]
