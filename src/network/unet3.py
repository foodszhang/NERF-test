...
class FullScaleSkipConnect(nn.Module):
    def __init__(
        ...
        for ii, scale in enumerate(dec_scales):
            ...
            self.dec2dec_layers.append(
                dec2dec_layer(dec_ch, skip_ch, scale, fast_up=fast_up)
            )
        ...
        if am == CBAM:
            cbam = CBAM(concat_ch)
            self.AM = nn.Sequential(nn.ReLU(), cbam)
        else:
            self.AM = nn.Identity()
    ...
    def forward(self, en_maps, dec_maps=None):
        ...
        return self.AM(self.fuse_layer(self.droupout(torch.cat(out, 1))))
        ...
        if aux_losses > 0:
            ...
            for ii in layer_indices:
                ...
                self.aux_head.add_module(
                    f"aux_head{ii}", nn.Conv2d(ch, num_classes, 3, padding=1)
                )
            ...
            if self.aux_head is not None:
                for ii, de in enumerate(de_out[:-1]):
                    ...
                    if head_key in self.aux_head:
                        ...
                        pred[f"aux{ii}"] = self.resize(
                            self.aux_head[head_key](de), h, w
                        )
        return pred
